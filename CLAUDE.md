...
- **测试**：195/195 通过（`swift test`）。在 175 基线上新增 AdminPromoteTests 15 条覆盖：mirror→item promote / id 冲突 INSERT OR IGNORE / 拒绝 standalone & primary 模式 / 空 pull_cursor 只写 self lineage / serve_host+port override / 用户未知字段保留 / self 在 pull_cursor 边界 / **P1 stamp ingested_at_ns**（own null 被 stamp 单调 + already 不二次 + mirror 异常 nil 兜底）/ **P2 blob 预检**（默认拒绝 + allow-missing 通过 + bytes 齐全过 + 软删行不参与预检）。注意 PullWorker HTTP 端到端测试**已知偶发并发 flake**（端口/DB 竞争），单跑 `--filter PullWorker` 必绿
