...
- **M3 第三刀 部分完成**（PR#4 + PR#7 已 merge）：
  ...
  - **v5 migration**：`primary_lineage(device_id TEXT, started_at_ns INTEGER, ended_at_ns INTEGER, PK(device_id, started_at_ns))`。audit-push 后续读 `SELECT DISTINCT device_id FROM primary_lineage` 拿"曾被本机视角看作 primary 的所有 device"，把它们排除出 Continuity-dedup 候选——多 primary lineage 下 `origin != self` 启发式偏宽松（PR#7 只埋数据，audit 端切换留待下一 PR）
  - 仍未做：`migrate-primary` / blob 懒拉（M3 第三刀剩余部分）+ audit-push 切到 lineage-aware 候选过滤
...
- **测试**：183/183 通过（`swift test`）。在 175 基线上新增：AdminPromoteTests 8 条覆盖（mirror→item promote / id 冲突 INSERT OR IGNORE / 拒绝 standalone & primary 模式 / 空 pull_cursor 只写 self lineage / serve_host+port override / 用户未知字段保留 / self 在 pull_cursor 边界）。注意 PullWorker HTTP 端到端测试**已知偶发并发 flake**（端口/DB 竞争），单跑 `--filter PullWorker` 必绿
- **依赖**：GRDB 7.10.0 + Hummingbird 2.22.0 + HummingbirdTLS（SwiftPM 远程依赖）
- **下一站**：M3 第三刀剩余部分——`migrate-primary` 子命令（计划内换 primary：停老 primary → VACUUM INTO snapshot → rsync → 启新 primary）+ blob 懒拉 + audit-push 切到 lineage-aware 候选过滤（读 `primary_lineage` 把"曾经/现在作为 primary 的 device_id" 排除出 dedup 候选，详 `AuditPush.swift` 的 `TODO(promote-lineage)`）。详细拆解见 plans/...moonlit-wave.md
...
`Admin.promoteToPrimary` 把本机从 client 升级到 primary，全程一个 writer tx 包住——前 4 步原子，要么全成要么全不动。重要不变量：

1. **`item_mirror` → `item` 用 `INSERT OR IGNORE`，不是 REPLACE**。本机自家 own-origin item 比 mirror 行可信（mirror 是从老 primary 拉的，可能漏更新；自家 own 是源头）。冲突时 own 行赢，mirror 行被丢弃。回归测试 `promoteWithIDConflictSkipsMirrorRow` 保护这个不变量。
2. **被 promote 进来的 item `push_state` 强制设 'acked'**。`push_state` 是 NOT NULL DEFAULT 'pending' —— 如果不显式设 'acked'，INSERT 进来变成 pending，本机的（已经停了的）PushWorker 概念上会去推它们，但它们的目的地 primary 就是自己。强制 acked 让这些行从 sync 角度看是"终态"。
...
4. **`primary_lineage` 闭老任期行用 `started_at_ns=0`**，表示"未知何时开始"。audit-push 的查询是 `SELECT DISTINCT device_id FROM primary_lineage`，不依赖 started 的精确值，0 是安全的 sentinel。PK 是 `(device_id, started_at_ns)`，同一 device 多次任期可记，但 (id, 0) 行不会重复——`INSERT OR IGNORE` 兜底。
5. **`Config.write` 保留未知字段**。读原 JSON 成 dict、覆盖 Config 内部 5 个目标 key（serve / serve_host / serve_port / primary_url / pull）、写回。capture / tls / shared_secret_keychain_account / 用户手动加的注释 key 全部不丢。回归测试 `promotePreservesUserConfigFields`。
6. **不动 LaunchAgent**。CLI 子命令是单次 exit 的进程，不应该 bootout/kickstart 自己。提示用户手动 `launchctl kickstart -k`。
