...
**四个 self-hosted-on-dev-machine 特有的坑（都已解，别回退）**：
...
3. **runner 的 `python3` 是 `/usr/bin/python3` (3.9)，不是你 shell 里的 homebrew 3.14**。同一台机器两个 python，PATH 差异跟坑 1 同源（LaunchAgent 不 source shell profile）。差异会咬人的地方：**3.13+ 容忍 `python3 -c` 里统一的前导缩进，3.9 直接 `IndentationError`**。所以 workflow 里的内联 python **不许带缩进**——要么整段顶到列 0（YAML block scalar 会剥掉公共缩进，写在 `run:` 的基准列即可），要么先 heredoc 写成独立 `.py` 文件再调。2026-08-12 run 31579493741 就是栽在这：本机测试全绿（3.14 容忍），CI 六次重试全挂。**本机跑通不等于 CI 跑通，验 CI 脚本请把 PATH 收成 `/usr/bin:/bin` 再测。**

4. **`SPARKLE_ED_PRIVATE_KEY` secret 必须填**。Sparkle EdDSA 私钥在 MBP login keychain（`generate_keys --account duopaste` 生成，service `https://sparkle-project.org`，account `duopaste`），对应嵌客户端的公钥 `5Ws5rSunj3IH...`。导出：`security find-generic-password -w -a duopaste -s 'https://sparkle-project.org' | gh secret set SPARKLE_ED_PRIVATE_KEY --repo jizhi0v0/duo-paste`。**必须 `-a duopaste`**——不带 account 的 `security` 可能抓到 default `ed25519` account 的老密钥（public key 不同），签出的 DMG 客户端校验 "improperly signed" 失败。

**release asset 的成败以 GitHub API 为准，不看上传步骤的退出码**。`Create release` 走 self-hosted runner 的本机代理，12MB DMG 传完后响应端断连是常态——2026-08-12 run 31577953326 报 `other side closed` 判失败，但 asset 其实完整落地（下载回来 `hdiutil verify` + `spctl` + `stapler validate` 全过）。所以那步是 `continue-on-error`，由后面的 `Verify + repair release asset` 按 API 事实收敛：release 在不在 → asset 在不在 → `state=uploaded` → **字节数等于 appcast 的 `enclosure length`**，缺什么补什么，6 次退避后仍不成才真失败。字节数这条最要紧：Sparkle 拿 length 跟下载到的字节比，传一半的 asset 会让所有客户端更新失败而 release 页面看着正常。反过来，本地 DMG 跟 appcast length 对不上是**硬失败不补传**——那说明两个 job 看的不是同一个文件，补传只会让签名配错字节。另外 **asset 名字进的是 query string，`+` 在那里表示空格**，必须 `urllib.parse.quote(..., safe="")` 编码，否则 `DuoPaste-…-beta+abc.dmg` 会被 GitHub 落成 `…-beta.abc.dmg`，appcast 指向的 URL 直接 404。
