...
- **M3 第三刀 部分完成**（PR#4 + PR#7 + PR#8 已 merge）：
  ...
  - 仍未做：`migrate-primary` / blob 懒拉（M3 第三刀剩余部分）
...
- **测试**：199/199 通过（`swift test`）。AdminPromoteTests 18 条覆盖：mirror→item promote / id 冲突 INSERT OR IGNORE / 拒绝 standalone & primary 模式 / 空 pull_cursor 只写 self lineage / serve_host+port override / 用户未知字段保留 / self 在 pull_cursor 边界 / **stamp ingested_at_ns**（own null 被 stamp 单调 + already 不二次 + mirror 异常 nil 兜底）/ **blob 预检**（默认拒绝 + allow-missing 通过 + bytes 齐全过 + 软删行不参与预检）/ **daemon-running 安全检查**（在跑拒绝 + 不在跑放行）/ **嵌套 config 字段保留**（pull/capture 子 dict 未知 key 不丢）。注意 PullWorker HTTP 端到端测试**已知偶发并发 flake**（端口/DB 竞争），单跑 `--filter PullWorker` 必绿
- **依赖**：GRDB 7.10.0 + Hummingbird 2.22.0 + HummingbirdTLS（SwiftPM 远程依赖）
- **下一站**：M3 第三刀剩余部分——`migrate-primary` 子命令（计划内换 primary：停老 primary → VACUUM INTO snapshot → rsync → 启新 primary）+ blob 懒拉。详细拆解见 plans/...moonlit-wave.md
