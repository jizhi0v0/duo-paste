...
/// 验证 PullWorker.evictOnFull 注入到位 —— BlobStore.putVerifiedRetryingOnFull 走的就是
/// 这个 closure。`putVerified` 顺路成功不调 evictor（retryOnFull 仅 ENOSPC 才调），
/// 所以这里数 evictor 调用次数 = 0；测的是注入路径而非 retry 行为
@Test func pullWorkerCallsEvictOnFullClosureWhenWired() async throws {
    let db = try makeDB()
    let blobs = makeBlobs()
    let bytes = Data(repeating: 0xCC, count: 50)
    let sha = sha256Hex(bytes)
    let item = mkItem(id: "img-1", origin: "peer-X", ingestedAtNs: 100, blobSha: sha)
    let transport = StubSinceTransport(
        pages: [page(items: [item], nextNs: 100, nextID: "img-1", hasMore: false)],
        healthDeviceID: "peer-X"
    )
    let fetcher = StubBlobFetcher()
    await fetcher.set(sha, .found(bytes))

    // counter 用 actor 让 @Sendable closure 安全访问
    actor EvictCounter {
        private(set) var count = 0
        func increment() { count += 1 }
        func snapshot() -> Int { count }
    }
    let counter = EvictCounter()
    let evictOnFull: @Sendable () throws -> Bool = {
        // sync-from-async 不允许；用 Task 提交计数。put 成功不会到这里
