...
/// OCR Phase 2 跨设备同步：mini 端 OCR 完成后 ocr_state=done + text_full 写入，
/// 对端 PullWorker 通过 /since 拉到这条更新，对端 item 表 ocr_state + text_full
/// 同步落地，FTS5 索引 OCR text 让对端搜索可命中。
///
/// 这是端到端 SQL 层验证：**不**起 server，模拟 mini → MBP 的状态推送。
/// 完整 HTTP 路径已经被 PullWorkerTests 覆盖（INSERT OR REPLACE 走 ocr_state 列），
/// 这里专门钉死"OCR 结果跨设备 visible to search"语义。
...
@Test func pullWorkerApplyOverwritesPeerOCRStateAndTextFull() async throws {
    // 模拟：MBP 上预先有一条 mini-origin image（pending，没 OCR text）
    // → PullWorker 拉到 mini 上同 id 的 OCR done 更新 → MBP 上 ocr_state 变 done + text_full 写入
    let db = try makeDB()
    ...
    // mini 上 OCR 完成后这条 item 的状态：text_full 填好 + ocr_state=done + ingested_at_ns bump
    let oCRDone = Item(
        id: "mini-img-1",                       // 同 id
        originDevice: "mini-device",
        capturedAtNs: 100,
        ...
        sourceAppName: "ScreenCapture",
        preview: "[image 100KB]",
        textFull: "OCR result text 文本识别结果",
        blobSha256: "deadbeef" + String(repeating: "0", count: 56),
        blobSize: 100 * 1024,
        blobMime: "image/png",
        ocrState: .done
    ...
    // 验证：MBP 上同 id item 的 ocr_state 变 done + text_full 写入
    let after = try await db.pool.read { conn in
        try Item.filter(Column("id") == "mini-img-1").fetchOne(conn)
    }
    #expect(after?.ocrState == .done)
    #expect(after?.textFull == "OCR result text 文本识别结果")
    #expect(after?.ingestedAtNs == 500)
}

@Test func pullWorkerApplyTriggersFTSIndexForOCRText() async throws {
    // OCR 结果的 text_full 通过 INSERT OR REPLACE 写入后，FTS5 trigger（item_au）应当
    // 自动重 index。MBP 上搜 OCR text 应能命中
    let db = try makeDB()
    // 预置：MBP 已有这条 image 的 pending 行（text_full=nil）
    let prior = Item(
        id: "mini-img-2",
        originDevice: "mini-device",
        capturedAtNs: 200,
    ...
    // 拉到 mini 端 OCR done 更新（含一段独特的可搜词 "灯泡 lighting"）
    let oCRDone = Item(
        id: "mini-img-2",
        originDevice: "mini-device",
        capturedAtNs: 200,
        ...
        kind: .image,
        preview: "[image 50KB]",
        textFull: "灯泡 lighting fixture installation manual page 3",
        blobSha256: "cafebabe" + String(repeating: "0", count: 56),
        blobSize: 50 * 1024,
        ocrState: .done
    )
